...
class OTAUpdater:
    ...
    def __init__(
        ...
        self.exclude_files.append(new_version_dir)
    ...
    def _delete_old_version(self):
        ...
        self._rmtree(self.modulepath(self.main_dir), self.exclude_files)
    ...
    def _rmtree(self, directory, exclude_files=[]):
        for entry in os.ilistdir(directory):
            if entry[0] in exclude_files:
                continue
        ...
        if directory not in exclude_files:
            print("Deleting directory: ", directory)
            os.rmdir(directory)

    def _os_supports_rename(self) -> bool:
        return (
            False  # Rename dir into "/" raises an error "EPERM". So we use copy instead
        )
